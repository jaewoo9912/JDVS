...
interface vdmatb_vwrap_if;
  ...
  `vaxi_snps_active_mst_if(axi_if, m_card_axi, PORT1, CARD_AXI_PORT_PARAM)
  // -----------------------------------------------------------------------------
  // DUT-specific
  // -----------------------------------------------------------------------------
  ...
  function automatic void forceCardRLast_disable();
	  force m_card_axi.rlast = 1'b0;
	  forced_rlast_state = YES;
  endfunction:forceCardRLast_disable
  
  function automatic void forceCardRLast_Enable();
	  force m_card_axi.rlast = 1'b1;
	  forced_rlast_state = YES;
  endfunction:forceCardRLast_Enable
  
  function automatic void releaseCardRLast();
	  release m_card_axi.rlast;
	  forced_rlast_state = NO;
  endfunction:releaseCardRLast
