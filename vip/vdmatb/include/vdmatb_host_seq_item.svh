`ifndef __VDMATB_HOST_SEQ_ITEM_SVH__
`define __VDMATB_HOST_SEQ_ITEM_SVH__


class vdmatb_host_seq_item extends vmg_seq_item;
...
//logic [16-1:0]                                 dma_id;
  ...
  logic [`SVT_AXI_RESP_WIDTH-1:0]                q_rresp[$];
   ...
   rand logic                                           awready;
   rand logic                                           awvalid;
   rand logic [pdma_dut_pkg::HOST_ADDR_WIDTH-1:0]       awaddr;
   rand logic [pdma_dut_pkg::HOST_ADDR_WIDTH-1:0]       waddr[$];
   ...
   //rand logic [`SVT_AXI_SIZE_WIDTH-1:0]                 awsize;
   //rand logic [`SVT_AXI_LOCK_WIDTH-1:0]                 awlock;
   //rand logic [`SVT_AXI_PROT_WIDTH-1:0]                 awprot;
   //-----------------------------------------------------------------------
   ...
   rand logic                                           wvalid;
   rand logic                                           wlast;
   rand logic [HOST_DATA_WIDTH-1:0]                     wdata [$]; 
   rand logic [HOST_DATA_BYTE_WIDTH-1:0]                wstrb [$]; 
   //rand logic [SVT_AXI_ID_WIDTH_PARAM-1:0]              wid; 
   ...
   rand logic                                           arready;
   rand logic                                           arvalid;
   rand logic [pdma_dut_pkg::HOST_ADDR_WIDTH-1:0]       araddr; 
   rand logic [pdma_dut_pkg::HOST_ADDR_WIDTH-1:0]       raddr[$];
   ...
   //rand logic [`SVT_AXI_SIZE_WIDTH-1:0]                 arsize;
   //rand logic [`SVT_AXI_LOCK_WIDTH-1:0]                 arlock;
   //rand logic [`SVT_AXI_PROT_WIDTH-1:0]                 arprot;
  YesOrNo_t	need_fault_ar = NO;
   ...
   rand logic [HOST_DATA_WIDTH-1:0]                     rdata[$]; 
  ...
  `uvm_object_utils(vdmatb_host_seq_item)

  function new (string name="vdmatb_host_seq_item");
    super.new(name);
...
endclass:vdmatb_host_seq_item

function YesOrNo_t vdmatb_host_seq_item::isCompleted();
  YesOrNo_t result;

//  if(this.trans_status == COMPLETED_WO_CONSIDERING_FAULT) return(YES);
  return(NO);
...
`endif // __VDMATB_HOST_SEQ_ITEM_SVH__
