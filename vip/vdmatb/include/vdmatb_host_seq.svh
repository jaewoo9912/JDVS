`ifndef __VDMATB_HOST_SEQ_SVH__
`define __VDMATB_HOST_SEQ_SVH__



virtual class vdmatb_host_seq extends vaxi_slv_seq;
  ...
  vdmatb_scfg scfg;
	...
	function new(string name="vdmatb_host_seq");
		super.new(name);
	endfunction
...
endclass:vdmatb_host_seq




// TODO:Refactoring w/ small pcieces of methods
task vdmatb_host_seq::doSend();
  AxiSlvTrans_t found;
  ...
  forever begin
    foreach (rdata_prob[i]) rdata_prob[i] = FlipCoin(this.prob_slv_min_max);
      ...
      found.port_cfg.ruser_enable = 0;
      found.port_cfg.buser_enable = 0;
      if(this.tcfg.test_type == ASYMMETRIC_LATENCY_TEST) begin
        found.port_cfg.default_arready                         = 0;
        found.port_cfg.default_awready                         = 0;
        found.port_cfg.default_wready                          = 0;
...
function void vdmatb_host_seq::setupDelayType_IDEAL();
...
function void vdmatb_host_seq::setupDelayType_ON_DELAY_WO_RESP();
  this.rgs_addr_read_ready_delay_type    = RGS_PRESET;
...
function void vdmatb_host_seq::setupDelayType_FOR_REGRESSION();
  this.rgs_addr_read_ready_delay_type    = RGS_PRESET;
  this.preset_addr_read_ready_delay_type = RGS_AXI_ADDR_READ_READY_NORMAL_DELAY;
  this.min_addr_read_ready_delay_type    = RGS_AXI_ADDR_READ_READY_RANDOM_DELAY;
  this.max_addr_read_ready_delay_type    = RGS_AXI_ADDR_READ_READY_LONG_DELAY;
...
function void vdmatb_host_seq::setDefaultCfg();
...
function void vdmatb_host_seq::setupDelayType_AsymmetricLatency();
